...
class VariantRequest(BaseModel):
    ...
    genome: str
    chromosome: str
    sequence: Optional[str] = None  # Pre-fetched sequence from frontend
...
class Evo2Model:
    ...
    def analyze_single_variant(self, request: VariantRequest):
        ...
        # Use pre-fetched sequence from frontend if provided (avoids Modal network issues)
        if request.sequence:
            print("Using pre-fetched sequence from frontend")
            window_seq = request.sequence
            seq_start = max(0, variant_position - 1 - WINDOW_SIZE // 2)
        else:
            # Fallback: fetch from UCSC directly (may timeout in some Modal containers)
            print("No sequence provided, fetching from UCSC...")
            window_seq, seq_start = get_genome_sequence(
                position=variant_position,
                genome=genome,
                chromosome=chromosome,
